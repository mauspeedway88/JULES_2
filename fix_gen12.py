...
wikipedia.set_user_agent("EducationalBot/4.0 (Contact: edubot@example.com) wikipedia/1.4.0")
...
def is_educational(sentence):
    if len(sentence.split()) < 15: return False
    noise = ["wikimedia commons", "archivo:", "isbn", "doi:", "cita requerida", "consultado", "fecha", "siglo", "nació", "murió"]
    if any(n in sentence.lower() for n in noise): return False
    ...
    if not any(tw in sentence.lower() for tw in tech_words): return False
    return True
...
def extract_keywords(text):
    ...
    stop = {'el', 'la', 'los', 'las', 'un', 'una', 'para', 'como', 'este', 'esta', 'se', 'su', 'sus',
            ...
            'fue', 'fueron', 'ser', 'estar', 'ha', 'han', 'lo', 'le', 'les', 'me', 'te', 'nos', 'os', 'al', 'del'}
...
def main():
    ...
    while queue and len(conceptos) < CANTIDAD_OBJETIVO:
        ...
        try:
            ...
            for res in resultados:
                ...
                if any(x in res.lower() for x in ['película', 'álbum', 'música', 'rey', 'santo', 'botánico', 'juego', 'banda', 'novela']): continue

                try:
                    page = wikipedia.page(res, auto_suggest=False)
                    if any(x in page.summary.lower() for x in ['película', 'álbum', 'rey', 'santo', 'juego', 'ficción', 'cantante']): continue
                        ...
                        if 35 <= len(words) <= 50 and 4 <= len(entry["keywords"]) <= 6:
                            ...
                            if len(conceptos) % 50 == 0:
                                ...
                                with open("MM_brain_44.json", "w", encoding="utf-8") as f:
                                    json.dump(conceptos, f, indent=4, ensure_ascii=False)
    ...
    while len(conceptos) < CANTIDAD_OBJETIVO:
         ...
         try:
            resultados = wikipedia.search(tema_actual, results=3)
            for res in resultados:
                if res in visitadas: continue
                visitadas.add(res)
                try:
